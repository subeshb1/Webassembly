...
class GoWorker {
    ...
    async validate(buffer) {
        ...
        try {
            this.go.argv = ["imageAscii.wasm", "-c", "-f", "Loreal-Paris-BMAG-Article-7-clear-skin-tips-for-your-best-complexion-yet-D.jpg", "-c=true"];
    ...
    async extractPage(buffer, page) {
        ...
        this.go.argv = ['imageAscii.wasm', 'trim', '-pages', String(page), '/test.pdf', '/first_page.pdf'];
        var st = Date.now();
        await this.go.run(this.instance);
        console.log('Time taken:', Date.now() - st);
    ...
    async beforeProcess(buffer) {
        ...
        if (!this.compiledModule) {
            let result = await WebAssembly.instantiateStreaming(fetch("imageAscii.wasm"), this.go.importObject);
